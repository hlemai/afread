...
static std::wstring str_to_wstr(const std::string& str)
{
    ...
    size_t wcstr_len = mbstowcs(temp,str.c_str(),(int)str.length());
    if (wcstr_len == 0) {
        return ret;
    }
	ret = temp;
...
*/
MACOSDriver::MACOSDriver(): m_file(NULL) {}

bool MACOSDriver::Open(const std::string& file_name)
{
...
void MACOSDriver::Read(void* data, size_t size, size_t* bytes_read)
{
	size_t w = 0;
	if (m_file != NULL)
	{
...
AffinityReader::ByteBuffer MACOSDriver::Read(size_t size)
{
	size_t                     bytes_read;
...
bool MACOSDriver::Seek(uint64_t offset)
{
	bool ok = true;
	if (m_file != NULL)
	{
...
AffinityReader::ByteBuffer MACOSDriver::DecompressZip(const AffinityReader::ByteBuffer& buf)
{
	return afread::DecompressZip((void*)buf.data(), buf.size());
}

AffinityReader::ByteBuffer MACOSDriver::DecompressZstd(const AffinityReader::ByteBuffer& buf)
{
	return afread::DecompressZstd((void*)buf.data(), buf.size());
}

uint32_t MACOSDriver::Crc32(const AffinityReader::ByteBuffer& buf)
{
